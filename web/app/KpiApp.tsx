...
import { CatalogAdminPage } from "./CatalogImports";
...
import { KPI_EXPORT_COLUMNS, toKpiExportRows } from "./exportRows";
...
type Kpi = { week_slot:number; team_name:string; start_date:string; end_date:string; day_count:number; total_payroll:number; total_production:number; difference_vnd:number; evaluation:string; actual_production:number; actual_vs_plan:number; production_rows_entered:number };
...
type PageId = "home"|"danh-muc-to"|"nhan-su"|"giao-viec"|"san-luong"|"danh-gia"|"pgv"|"pgvcnch"|"users"|"danh-muc-gia";

const PAGE_TITLES:Record<PageId,string> = {
  home:"🏠 Tổng Quan", "danh-muc-to":"🏢 Danh Mục Tổ", "nhan-su":"👥 Nhân Sự & Quỹ Lương",
  "giao-viec":"📋 Giao Việc & Hòa Vốn", "san-luong":"📈 Đánh Giá Sản Lượng", "danh-gia":"📊 Đánh Giá KPI", pgv:"📄 Phiếu Giao Việc (PGV)",
  pgvcnch:"👷 Phiếu PGV CNCH", users:"🔐 Phân Quyền Người Dùng",
  "danh-muc-gia":"💰 Danh Mục, Đơn Giá & Bảng Lương",
};
const money = new Intl.NumberFormat("vi-VN",{style:"currency",currency:"VND",maximumFractionDigits:0});
// Khối lượng KHÔNG phải tiền. Định mức hòa vốn và khối lượng yêu cầu đều tính bằng đơn vị của
// công việc (md, m², kg…), nên phải kèm đơn vị chứ không được định dạng thành VNĐ.
const qty = new Intl.NumberFormat("vi-VN",{maximumFractionDigits:1});
const withUnit=(value:unknown,unit:string|null,suffix="")=>`${qty.format(Number(value)||0)} ${unit||""}${suffix}`.trim();
// Lãi thì thêm dấu +, cho đọc lướt vẫn phân biệt được với lỗ.
const signedMoney=(value:unknown)=>{const n=Number(value)||0;return (n>0?"+":"")+money.format(n)};
...
export default function KpiApp(){
  ...
  // null = xem tất cả các tuần. Việc chưa gộp vào tuần nào KHÔNG nằm trong KPI (quyết định 11/08/2026).
  ...
  async function loadAll(){
    ...
    // trả về các tổ giám sát được giao (migration 044). Không lọc lại ở đây — lọc ở giao diện
    // thì chỉ là giấu đi, số vẫn đã về tới trình duyệt.
    if(current.role_code==="ADMIN"||current.role_code==="GIAM_SAT"){
      // Lần tải đầu luôn lấy TẤT CẢ các tuần, đúng bằng giá trị mặc định của bộ lọc.
      // Không đọc kpiWeek ở đây để loadAll không phụ thuộc state — việc lọc do changeKpiWeek lo.
      const kpiResult=await supabase.rpc("get_kpi_evaluation",{p_week_slot:null});
  ...
  // Đổi tuần KPI: nạp lại RIÊNG bảng KPI, không tải lại toàn bộ màn hình.
  // Gọi từ sự kiện onChange chứ không đặt trong useEffect — tránh phụ thuộc vòng vào loadAll.
  async function changeKpiWeek(week:number|null){
    setKpiWeek(week);
    const {data,error:kpiError}=await supabase.rpc("get_kpi_evaluation",{p_week_slot:week});
    if(kpiError){ setNotice(`KPI đang khóa an toàn: ${kpiError.message}`); setKpis([]); return; }
    setKpis((data||[]) as Kpi[]);
  }
  ...
  const navMain:[PageId,string,string][]=[
    ["home","🏠","Tổng Quan"],["danh-muc-to","🏢","Danh Mục Tổ"],["nhan-su","👥","Nhân Sự & Quỹ Lương"],["giao-viec","📋","Giao Việc & Hòa Vốn"],["san-luong","📈","Đánh Giá Sản Lượng"],["danh-gia","📊","Đánh Giá KPI"],
  ];
  ...
  return <div id="app">
    ...
    <aside id="sidebar" className={menuOpen?"open":""}>
      <div className="logo-area"><img className="brand-logo" src="/project-logo.png" alt="VINCONS"/><div className="app-title">THI CÔNG KPI MEP</div><div className="app-sub">Quản lý Giao Việc & Định Mức MEP</div></div>
      ...
      <nav><div className="nav-section">Menu Chính</div>{navMain.map(item=><NavItem key={item[0]} item={item} active={page===item[0]} onClick={()=>navigate(item[0])}/>) }
        ...
        {isAdmin&&<><div className="nav-section">Quản Trị</div><NavItem item={["users","🔐","Phân Quyền"]} active={page==="users"} onClick={()=>navigate("users")}/><NavItem item={["danh-muc-gia","💰","Đơn Giá & Lương"]} active={page==="danh-muc-gia"} onClick={()=>navigate("danh-muc-gia")}/></>}
      </nav>
...
function Brand(){return <div className="auth-brand"><img src="/project-logo.png" alt="VINCONS"/><div><strong>THI CÔNG KPI MEP</strong><span>Quản lý Giao Việc & Định Mức MEP</span></div></div>}
...
type PageProps={page:PageId;kpiWeek:number|null;changeKpiWeek:(v:number|null)=>void;identity:Identity|null;teams:Team[];workers:Worker[];jobs:Job[];weeks:Week[];sharedWeeks:SharedWeek[];kpis:Kpi[];profiles:Profile[];teamFilter:string;setTeamFilter:(v:string)=>void;weekFilter:string;setWeekFilter:(v:string)=>void;selectedJobs:string[];setSelectedJobs:(v:string[])=>void;pgvTeam:string;setPgvTeam:(v:string)=>void;pgvWeek:string;setPgvWeek:(v:string)=>void;receiveDate:string;setReceiveDate:(v:string)=>void;navigate:(v:PageId)=>void;openTeam:()=>void;openJob:()=>void;editJob:(job:Job)=>void;reload:()=>Promise<void>;toast:(v:string)=>void};
function PageContent(props:PageProps){switch(props.page){case"home":return <Home {...props}/>;case"danh-muc-to":return <TeamsPage {...props}/>;case"nhan-su":return <WorkersPage {...props}/>;case"giao-viec":return <JobsPage {...props}/>;case"san-luong":return <ProductionPage {...props}/>;case"danh-gia":return <KpiPage {...props}/>;case"pgv":return <PgvPage {...props}/>;case"pgvcnch":return <PgvCnchPage {...props}/>;case"users":return <UsersPage {...props}/>;case"danh-muc-gia":return <CatalogPage {...props}/>;}}

function Home({identity,teams,jobs,kpis,navigate,openJob}:PageProps){// KPI giờ trả một dòng cho MỖI (tổ × tuần), nên phải đếm theo TỔ chứ không đếm dòng,
const failedTeams=new Set(kpis.filter(k=>Number(k.difference_vnd)<0).map(k=>k.team_name));
const passedTeams=new Set(kpis.map(k=>k.team_name).filter(n=>!failedTeams.has(n)));
const passed=passedTeams.size,failed=failedTeams.size;return <><div className="vc-hero"><div className="hero-title">Xin chào, {identity?.full_name||"Admin"}!</div><div className="hero-sub"><img className="brand-logo" src="/project-logo.png" alt=""/> Hệ thống Quản lý KPI - MEP VINCONS</div><div className="hero-date">📅 {new Date().toLocaleDateString("vi-VN",{weekday:"long",year:"numeric",month:"long",day:"numeric"})}</div></div><div className="stats-grid"><Stat tone="green" value={passed} label="🟢 Tổ ĐẠT KPI"/><Stat tone="red" value={failed} label="🔴 Tổ KHÔNG ĐẠT"/><Stat value={jobs.length} label="📋 Việc đang giao"/><Stat tone="cyan" value={teams.filter(t=>t.is_active).length} label="🏢 Tổ hoạt động"/></div><div className="card"><div className="card-header"><span className="ch-icon">⚠️</span><span className="ch-title">Cảnh báo KPI - Cần xử lý</span></div><div className="card-body">{failed?<div className="kpi-alert-list">{kpis.filter(k=>Number(k.difference_vnd)<0).map(k=><div className="kpi-alert" key={`${k.week_slot}-${k.team_name}`}><span>🔴</span><div><strong>{k.team_name} · Tuần {k.week_slot}</strong><small>Chênh lệch: {money.format(Number(k.difference_vnd))}</small></div><button className="btn btn-sm btn-secondary" onClick={()=>navigate("danh-gia")}>Xem</button></div>)}</div>:<div className="empty-compact">🟢 Tất cả các Tổ đang đạt định mức KPI hoặc chưa có dữ liệu đánh giá.</div>}</div></div><div className="card"><div className="card-header"><span className="ch-icon">⚡</span><span className="ch-title">Thao Tác Nhanh</span></div><div className="card-body quick-grid"><button className="btn btn-primary" onClick={openJob}>➕ Giao Việc Mới</button><button className="btn btn-secondary" onClick={()=>navigate("danh-gia")}>📊 Xem KPI</button><button className="btn btn-cyan" onClick={()=>navigate("pgv")}>📄 Phiếu PGV</button><button className="btn btn-secondary" onClick={()=>navigate("pgvcnch")}>👷 Phiếu CNCH</button></div></div></>}
...
function WeekToolbar({teamWeeks,sharedWeeks,selected,assign,unassign,reload,toast,canManage}:{teamWeeks:Week[];sharedWeeks:SharedWeek[];selected:number;assign:(slot:number)=>Promise<void>;unassign:()=>Promise<void>;reload:()=>Promise<void>;toast:(v:string)=>void;canManage:boolean}){
  ...
  async function save(){if(!start||!end){toast("Hãy nhập đủ ngày theo dd/mm/yyyy.");return}setSaving(true);const {error}=await supabase.rpc("upsert_shared_work_week",{p_week_slot:Number(slot),p_start_date:start,p_end_date:end});setSaving(false);if(error){toast(operationError(error.message));return}setShow(false);toast(`Đã lưu Tuần ${slot} dùng chung cho các Tổ.`);await reload()}
...
function KpiPage({identity,kpis,kpiWeek,changeKpiWeek,sharedWeeks,toast}:PageProps){
  // Vai trò khác thì nói thẳng là không có quyền, thay vì để bảng rỗng khiến người dùng
  // tưởng chưa có dữ liệu. Đây chỉ là lời giải thích: chặn thật nằm ở database.
  if(!["ADMIN","GIAM_SAT"].includes(identity?.role_code||"")) return <Empty icon="🔒" title="Chỉ Quản trị viên và Giám sát được xem KPI"/>;
  return <><div className="card no-print"><div className="card-body filters-row"><select className="form-control" value={kpiWeek??""} onChange={e=>void changeKpiWeek(e.target.value?Number(e.target.value):null)}><option value="">Tất cả các tuần</option>{[1,2,3,4].map(n=>{const w=sharedWeeks.find(x=>x.week_slot===n);return <option key={n} value={n}>{w?`Tuần ${n}: ${viDate(w.start_date)}–${viDate(w.end_date)}`:`Tuần ${n} (chưa tạo)`}</option>})}</select><span className="loading-text">Chỉ tính các việc đã gộp vào tuần. Việc chưa gộp không nằm trong KPI.</span>{identity?.role_code==="ADMIN"&&<button className="btn btn-secondary" disabled={!kpis.length} onClick={()=>{if(!kpis.length){toast("Không có dòng nào để xuất.");return}downloadXlsx(buildTableXlsx("DanhGiaKPI",KPI_EXPORT_COLUMNS,toKpiExportRows(kpis)),asciiFileName(`DanhGiaKPI_${kpiWeek?`Tuan${kpiWeek}`:"CacTuan"}_${isoToday()}`)+".xlsx");toast(`Đã xuất ${kpis.length} dòng.`)}}>⬇️ Xuất Excel ({kpis.length} dòng)</button>}</div></div><div className="section-divider"><span>Bảng Đánh Giá KPI Tổng Hợp</span></div><div className="kpi-grid">{kpis.map(k=><article className={`kpi-card ${Number(k.difference_vnd)<0?"danger":"safe"}`} key={`${k.week_slot}-${k.team_name}`}><div className="kpi-team"><span>{Number(k.difference_vnd)<0?"🔴":"🟢"}</span><strong>{k.team_name}</strong><span className="badge badge-week">TUẦN {k.week_slot}</span><span className={`badge ${Number(k.difference_vnd)<0?"badge-red":"badge-green"}`}>{k.evaluation}</span></div><div className="kpi-values"><div><span>Quỹ lương</span><b>{money.format(Number(k.total_payroll))}</b></div><div><span>Sản lượng</span><b>{money.format(Number(k.total_production))}</b></div><div><span>Chênh lệch</span><b>{money.format(Number(k.difference_vnd))}</b></div></div><div className={`kpi-actual ${k.production_rows_entered===0?"pending":Number(k.actual_vs_plan)<0?"behind":"ontrack"}`}><span>Sản lượng thực tế</span>{k.production_rows_entered===0?<b className="muted">chưa có dữ liệu nhập</b>:<><b>{money.format(Number(k.actual_production))}</b><small>{Number(k.actual_vs_plan)<0?"🔴":"🟢"} so với sản lượng giao: {signedMoney(k.actual_vs_plan)}</small></>}</div><div className="kpi-bar"><div className={`kpi-bar-fill ${Number(k.difference_vnd)<0?"danger":"safe"}`} style={{width:`${Math.min(100,Math.max(5,Number(k.total_production)/(Number(k.total_payroll)||1)*100))}%`}}/></div></article>)}{!kpis.length&&<Empty icon="📊" title="KPI đang khóa an toàn hoặc chưa đủ dữ liệu"/>}</div></>}
...
function ProductionPage({identity,teams,sharedWeeks,teamFilter,toast}:PageProps){
  // Ai vao duoc man hinh nay do database quyet dinh (get_production_evaluation). O day chi
  // giai thich cho vai tro khong co quyen, thay vi de bang rong trong nhu chua co du lieu.
  if(!["ADMIN","GIAM_SAT","TO_TRUONG"].includes(identity?.role_code||"")) return <Empty icon="🔒" title="Chỉ Quản trị viên, Giám sát và Tổ trưởng được xem trang này"/>;
  const isAdmin=identity?.role_code==="ADMIN";
  return <ProductionPanel teams={teams} sharedWeeks={sharedWeeks} canUnlock={isAdmin} canExport={isAdmin} defaultTeam={teamFilter} notify={toast}/>;
}
function UsersPage({teams,workers,toast}:PageProps){return <UsersAdminPanel teams={teams} workers={workers} notify={toast}/>;}
function CatalogPage({identity,reload,toast}:PageProps){if(identity?.role_code!=="ADMIN")return <Empty icon="🔒" title="Chỉ Quản trị viên được xem trang này"/>;return <CatalogAdminPage notify={toast} onChanged={reload}/>;}
