...
type CatalogHit={price_item_id:string;category_name:string;content:string;unit:string|null;
  calc_price:number|null;so_tu_khop:number;tong_so_tu:number;khop_het:boolean;ly_do?:string};
type Cap1Hit={category_name:string;so_muc:number};
...
export function JobEditorModal({teams,catalog,defaultTeam,job,close,done,notify}:{teams:Team[];catalog:Catalog[];defaultTeam:string;job:EditableJob|null;close:()=>void;done:()=>Promise<void>;notify:(v:string)=>void}){
  ...
  // Ô hỏi AI tìm hạng mục. Tìm bằng SQL chạy TRƯỚC ở phía máy chủ; AI chỉ được gọi khi tìm
  // thường không ra dòng nào khớp hết từ khóa, và chỉ được CHỌN trong danh sách có thật.
  // Khóa OpenAI nằm ở secret của Worker nên trình duyệt không bao giờ thấy nó.
  const [aiQuery,setAiQuery]=useState("");
  const [aiRows,setAiRows]=useState<CatalogHit[]>([]),[aiCap1,setAiCap1]=useState<Cap1Hit[]>([]);
  const [aiBusy,setAiBusy]=useState(false),[aiNote,setAiNote]=useState(""),[aiSource,setAiSource]=useState("");

  async function askCatalog(){
    const q=aiQuery.trim();
    if(q.length<2){setAiNote("Gõ ít nhất 2 ký tự mô tả công việc.");return}
    setAiBusy(true);setAiNote("");setAiRows([]);setAiCap1([]);setAiSource("");
    try{
      const {data}=await supabase.auth.getSession();
      const token=data.session?.access_token;
      if(!token)throw new Error("Phiên đăng nhập đã hết hạn.");
      const res=await fetch("/api/ai/catalog-search",{method:"POST",
        headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},
        body:JSON.stringify({query:q})});
      const kq=await res.json();
      if(!res.ok)throw new Error(operationError(kq.error||"Không tìm được."));
      setAiRows((kq.ket_qua||[]) as CatalogHit[]);
      setAiCap1((kq.cap1||[]) as Cap1Hit[]);
      setAiSource(kq.nguon||"");
      if(kq.ghi_chu)setAiNote(kq.ghi_chu);
      else if(!(kq.ket_qua||[]).length)setAiNote("Không tìm thấy hạng mục nào khớp.");
    }catch(e){setAiNote(e instanceof Error?e.message:"Không tìm được.")}
    finally{setAiBusy(false)}
  }

  function chonHangMuc(hit:CatalogHit){
    // Đặt cả Cấp 1 và Cấp 2: đơn giá tra theo CẶP, thiếu một vế là không tra được.
    setCategory(hit.category_name);
    setContent(hit.content);
    setAiRows([]);setAiCap1([]);setAiNote("");
    notify(`Đã chọn: ${hit.category_name} › ${hit.content}`);
  }
