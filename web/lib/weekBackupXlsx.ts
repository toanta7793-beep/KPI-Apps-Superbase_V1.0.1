...
export function buildWeekBackupXlsx(rows:Record<string,unknown>[],production:Record<string,unknown>[]=[]){
  ...
  // Sản lượng theo ngày đi ở SHEET RIÊNG. Nó là quan hệ một-nhiều với việc (mỗi việc nhiều
  // ngày) nên không nhét chung một bảng phẳng được, mà bỏ ra ngoài thì xóa tuần sẽ xóa mất
  // số tổ trưởng đã nhập và file backup không có gì để phục hồi.
  const prodColumns=["job_id","content","location","unit","work_date","quantity","is_locked","locked_at","locked_by","created_at","created_by","team_name"];
  const prodLabels=["Job ID","Nội dung công việc","Vị trí","Đơn vị","Ngày","Khối lượng hoàn thành","Đã khóa","Khóa lúc","Người khóa","Tạo lúc","Người tạo","Tổ"];
  ...
  const buildSheet=(cols:string[],heads:string[],data:Record<string,unknown>[])=>{
    const values=[heads,...data.map(row=>cols.map(key=>row[key]))];
    const lastColumn=columnName(cols.length-1);
  ...
  const sheet=buildSheet(columns,labels,rows);
  const prodSheet=buildSheet(prodColumns,prodLabels,production);
  return zipSync({
    ...
    "xl/workbook.xml":strToU8('<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="BACKUP_TUAN" sheetId="1" r:id="rId1"/><sheet name="SAN_LUONG_NGAY" sheetId="2" r:id="rId3"/></sheets></workbook>'),
    "xl/_rels/workbook.xml.rels":strToU8('<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet2.xml"/></Relationships>'),
    ...
    "xl/worksheets/sheet2.xml":strToU8(prodSheet),
  },{level:6});
}
