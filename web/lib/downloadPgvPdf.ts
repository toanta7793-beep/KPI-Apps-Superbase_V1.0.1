...
// Dùng chung với bản in HTML: hai bản in cùng một phiếu phải ra cùng số dòng.
import { cnchDisplay, mergePgvRows } from "../app/pgvRows.ts";
type Job={id:string;group_code:string|null;content:string;location:string|null;quantity:number;unit:string|null;start_date:string;end_date:string;target_daily:number|null;count_leader:number;count_worker1:number;count_worker2:number;count_worker3:number;count_helper:number};
...
function bocMotDoan(font:PDFFont,text:string,size:number,width:number){
...
// Tôn trọng xuống dòng CỨNG rồi mới bọc theo bề rộng ô.
// Cần vì phiếu gộp mã nhóm in mỗi hạng mục một dòng riêng trong cùng một ô; nếu để rơi thẳng
// vào nhánh /\s+/ ở trên thì ký tự xuống dòng chỉ còn là khoảng trắng và mọi hạng mục dính
// liền nhau thành một khối chữ.
export function split(font:PDFFont,text:string,size:number,width:number){
  const out:string[]=[];
  for(const doan of safe(text).split("\n")){
    const dong=bocMotDoan(font,doan,size,width);
    if(dong.length)out.push(...dong);else out.push("");
  }
