-- HOÀN TÁC 056_pgv_cnch_group_code.sql
--
-- Bỏ cột group_code khỏi danh sách việc của get_pgv_cnch.
--
-- ⚠️ Sau khi chạy, ô chọn phân công ở PGV CNCH mất đầu mục gộp (giao diện không còn biết
-- việc nào cùng nhóm). Các phân công gộp ĐÃ LƯU vẫn còn nguyên trong database và vẫn in ra
-- đúng, vì nhãn nằm ở content_label chứ không suy ra từ group_code.
-- Không mất dữ liệu: đây chỉ là danh sách cột của một hàm đọc.
