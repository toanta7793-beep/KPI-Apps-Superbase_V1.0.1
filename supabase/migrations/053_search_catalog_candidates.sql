-- 053_search_catalog_candidates.sql
--
-- Nền cho tính năng "AI tìm hạng mục công việc". Đây là BƯỚC MỘT: tìm bằng database.
--
-- Kiến trúc lai, và thứ tự này quan trọng:
--   1. Tìm không dấu ở đây. Ra kết quả khớp hết từ khóa  -> XONG, không gọi AI.
--   2. Không ra gì -> lấy danh sách ứng viên rộng hơn từ chính hàm này, rồi mới đưa AI xếp hạng.
--
-- Vì sao AI KHÔNG được tự sinh tên hạng mục: đơn giá tra theo cặp (hạng mục cấp 1 + nội dung)
-- khớp DUY NHẤT. Một cái tên do mô hình bịa ra sẽ không tra được đơn giá, và người dùng chỉ
-- phát hiện khi bấm Lưu và bị từ chối. Nên AI chỉ được CHỌN TỪ danh sách hàm này trả về —
-- mọi thứ nó nói tới đều là dòng có thật trong bảng đơn giá.
--
-- Hàm này cũng dùng được độc lập: nó tốt hơn hẳn tìm-khớp-hết-từ-khóa hiện tại vì cho điểm
-- theo SỐ TỪ KHỚP, nên gõ thiếu hoặc thừa một từ vẫn ra kết quả gần đúng.
