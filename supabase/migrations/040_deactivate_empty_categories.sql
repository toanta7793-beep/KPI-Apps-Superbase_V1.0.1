-- 040_deactivate_empty_categories.sql
--
-- Theo yêu cầu vận hành ngày 11/08/2026: hạng mục Cấp 1 rỗng phải biến mất khỏi danh sách chọn.
--
-- Trước đây khi nạp file đơn giá mới, các DÒNG cấp 2 không còn trong file thì chuyển
-- "ngừng dùng", nhưng HẠNG MỤC Cấp 1 thì không đụng tới. Kết quả: người giao việc vẫn thấy
-- hạng mục cũ trong dropdown, bấm vào thì không có công việc nào bên trong.
-- Trên staging đã tích lại 7 hạng mục trong khi chỉ 5 hạng mục còn dòng đang dùng.
--
-- Cách xử lý: chuyển hạng mục sang is_active = false thay vì xóa cứng.
-- get_catalog_cap1 vốn đã lọc theo is_active nên với người dùng thì nó biến mất y như bị xóa,
-- còn lịch sử các việc cũ vẫn tra ngược được. Nạp lại file có hạng mục đó thì nó sống lại.
--
-- Vẫn giữ nguyên tắc như với dòng đơn giá: hạng mục còn việc CHƯA XÓA tham chiếu thì GIỮ LẠI,
-- để không làm hỏng phần tra cứu của việc đang chạy.

-- Dọn một lần cho dữ liệu đang có, để không phải chờ tới lần nạp file tiếp theo.
update public.work_categories c
   set is_active = false, updated_at = now()
 where c.is_active
   and not exists(select 1 from public.price_items p where p.category_name = c.name and p.is_active)
   and not exists(select 1 from public.jobs j where j.deleted_at is null and j.category_name = c.name);
...
begin
  ...
  -- Hạng mục Cấp 1 không còn dòng cấp 2 nào đang dùng thì cũng ngừng dùng luôn, nếu không
  -- nó vẫn nằm trong danh sách chọn lúc giao việc mà bấm vào thì rỗng.
  -- Vẫn giữ lại nếu còn việc chưa xóa tham chiếu, đúng nguyên tắc như với dòng đơn giá.
  update public.work_categories c
     set is_active = false, updated_at = now(), updated_by = auth.uid()
   where c.is_active
     and not exists(select 1 from public.price_items p
                    where p.category_name = c.name and p.is_active)
     and not exists(select 1 from public.jobs j
                    where j.deleted_at is null and j.category_name = c.name);
  get diagnostics v_dead_cats = row_count;
  ...
  return jsonb_build_object(
    ...
    'deactivated_category_count',v_dead_cats,
